...
def productpersistent(a,b,N):   
    betap_node = np.random.beta(a, b, size = N) # random generate from beta distribution
    ps = np.zeros((N,N))
    for i in range(N):
        for j in range(i+1,N):
            ps[i,j] = betap_node[i]*betap_node[j]
    return(ps)
...
phat2 = np.sum(M1vector_even)/len(M1vector_even) 
phat = phat2**.5
p2hat2 = (np.sum(M2vector_even))/(timestephalf)
p2hat = p2hat2**.5
truthmoments = firstandsecond(parameter)

results = [phat, p2hat,M1vector_even[0]**.5,M2vector_even[0]**.5,truthmoments[0], truthmoments[1],parameter[0], parameter[1]]


filename = "WBetaproductpersistentestimationvector-N"+str(para1)+"-timestep"+str(para2)+"-rep"+str(para3)+".txt"
